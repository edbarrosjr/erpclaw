# Meta / Transversal — `glue-meta`

> Spec funcional por ação. Gerada de `scripts/glue-meta/db_query.py`. 4 funcionalidades · 4 ações.
...
| | |
|---|---|
| **Entradas** | --db-path (default ~/.glue/data.sqlite). Sem flags obrigatorias. |
...
| **Efeitos colaterais** | nenhum (leitura): apenas le sqlite_master e a tabela company, lista ~/glue/skills e le arquivos SKILL.md. |
...
Avalia o progresso por tier e recomenda a proxima skill/tier a instalar com o comando glue correspondente.
...
| **Efeitos colaterais** | nenhum (leitura): le sqlite_master/company e lista skills instaladas em ~/glue/skills. |
...
| | |
|---|---|
| **Entradas** | --db-path (default ~/.glue/data.sqlite); --company-id (opcional: usa empresa existente em vez de criar a Stark). |
...
| **Pré-condições** | Requer PyPika/glue_lib instalado e as skills referenciadas presentes em ~/glue/skills (ou nos pacotes v2 glue-ops/glue-growth); _find_skill_script localiza cada script e lanca erro 'not found' se ausente. Para --company-id, a empresa deve existir. |

## Setup Web Dashboard

**Objetivo.** Acao de setup do dashboard web, agora migrada para o addon externo glue-os-engine (stub de migracao nesta versao air-gapped).

### `setup-web-dashboard`

Stub que informa que o setup do dashboard web foi movido para o addon glue-os-engine (renomeado para os-setup-web-dashboard).
...
| **Saídas** | JSON de erro com status=error, error, missing_addon=glue-os-engine, old_action=setup-web-dashboard, new_action=os-setup-web-dashboard, note, since_version=4.0.0. |
