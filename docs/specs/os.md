# OS / Runtime (Fundação) — `glue-os`

> Spec funcional por ação. Gerada de `scripts/glue-os/db_query.py`. 6 funcionalidades · 8 ações.
...
| **Efeitos colaterais** | Nenhum (leitura) no banco principal: só lê arquivos do módulo (SKILL.md, init_db.py) e roda pytest em subprocesso. Não escreve tabelas, gl_entry/SLE/PLE, status nem audit_log; o runtime pode escrever em banco de teste isolado via GLUE_DB_PATH. |
...
| **Regras** | Erro (err) se src_root não for diretório existente. Varre glue-setup/init_schema.py atribuindo tabelas ao dono 'glue', depois todos os init_db.py sob src_root atribuindo ao módulo derivado do diretório; pula __pycache__/.git e o init_schema do core. Tabelas extraídas por regex CREATE TABLE IF NOT EXISTS. |
...
| | |
|---|---|
| **Entradas** | --module-path (obrigatório; deve conter init_db.py); --db-path (default ~/.glue/data.sqlite); --src-root (opcional; habilita checagem de conflito cross-module). |
...
| **Efeitos colaterais** | ESCREVE no banco: cria a tabela glue_schema_migration (se ausente) e INSERE 1 registro com status 'planned' (migration_type='create', ddl_statements, previous_schema snapshot, planned_at). Não toca tabelas de negócio, gl_entry/SLE/PLE nem audit_log. |
...
| | |
|---|---|
| **Entradas** | --migration-id (lido via getattr, obrigatório para o handler); --db-path (default ~/.glue/data.sqlite). OBS: o parser de db_query.py NÃO define --migration-id, então na prática o handler sempre recebe None e retorna o erro de flag obrigatória. |
...
| **Efeitos colaterais** | ESCREVE no banco: executa os DDL CREATE TABLE/INDEX da migração (cria tabelas reais) e ATUALIZA o status do registro em glue_schema_migration para 'applied' (ou 'failed' em erro). Não posta em gl_entry/SLE/PLE nem audit_log. |
| **Pré-condições** | Existir registro em glue_schema_migration com status 'planned' (criado por schema-plan) e o migration_id correspondente. |
...
| **Efeitos colaterais** | ESCREVE no banco: cria tabelas de backup (CREATE TABLE AS SELECT) para tabelas com dados, faz DROP das tabelas criadas pela migração, e ATUALIZA o registro em glue_schema_migration para 'rolled_back' com rolled_back_at. Não posta em gl_entry/SLE/PLE nem audit_log. |
| **Pré-condições** | Existir registro em glue_schema_migration com status 'applied' (aplicado por schema-apply) e o migration_id correspondente. |
...
| | |
|---|---|
| **Entradas** | --module-path (obrigatório; deve conter init_db.py); --db-path (default ~/.glue/data.sqlite). |
...
## Ações Movidas para Addon (glue-os-engine)

**Objetivo.** Stubs para ~29 ações de geração/deploy/evolução DGM que foram movidas para o addon externo glue-os-engine, retornando erro estruturado de addon ausente neste fork air-gapped.
...
Para qualquer uma das ~29 ações listadas em MOVED_ACTIONS_TO_ADDON, retorna erro estruturado indicando que a ação migrou para o addon glue-os-engine.
...
| **Saídas** | JSON de erro (print direto + exit 1) com status='error', error, missing_addon='glue-os-engine', old_action, new_action (nome os-prefixado), note e since_version='4.0.0'. |
